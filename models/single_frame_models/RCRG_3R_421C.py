...
Three relational layers with a progressively larger receptive field:
  R1: 4 cliques of 3 players each (sub-team groupings)
  R2: 2 cliques of 6 players each (one per team)
  R3: 1 clique of all 12 players (cross-team)
...
class GroupActivityClassifier(nn.Module):
    def __init__(self, person_model, num_classes, device):
        ...
        self.pool = nn.AdaptiveMaxPool2d((1, 896))
    ...
    def forward(self, x):
        ...
        team1 = self.pool(x[:, :6, :])             # (b, 1, 896)
        team2 = self.pool(x[:, 6:, :])             # (b, 1, 896)

        x = torch.cat([team1, team2], dim=1).view(b, -1)   # (b, 1792)
        return self.fc(x)
